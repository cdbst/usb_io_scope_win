...
NTSTATUS PHWIoScope_PnPRemoveDevice(PDEVICE_OBJECT pDevObj, PIRP pIrp, PIO_STACK_LOCATION pIoStackLocation){
	...
	pFidoNode = PHWIoScopeFltDevListMgr_FindSpecificDevNodeFromList(&globals.fltDevObjList, pDevObj, DEV_NODE_FIND_METHOD_FIDO_ADDR);

	if(pFidoNode != NULL){
		PHWIoScopeFltDevListMgr_DeleteFltDevNodeFromList(&globals.fltDevObjList, pFidoNode);
	}

	if(pDevExt->isCapturePacket){
		pAutoCapDevNode = PHWIoScopeAutoCapDevListMgr_CreateAutoCapDevNode(pDevExt->pHardwareID, pDevExt->hardwareIDSize);
		
		if(pAutoCapDevNode != NULL){
			PHWIoScopeAutoCapDevListMgr_InsertAutoCapDevNodeIntoList(&globals.autoCapDevList, pAutoCapDevNode);
		}	
	}

	if(pDevExt->pPdoName != NULL){
		ExFreePool(pDevExt->pPdoName);
		pDevExt->pPdoName = NULL;
	}

	if(pDevExt->pHardwareID!= NULL){
		ExFreePool(pDevExt->pHardwareID);
		pDevExt->pHardwareID = NULL;
	}
